...
def configure(conf, subdir=None):
    conf.env['CC'] = 'clang'
    conf.env['CXX'] = 'clang++'
    ...
    conf.env['INCLUDES'] = ['{}/include'.format(subdir) if subdir else 'include', '/usr/local/include']

    conf.env['GTEST'] = '{}/gtest-1.6.0'.format(subdir) if subdir else 'gtest-1.6.0'
...
def build_librosewood(bld, subdir=None):
    rwdir = '{}/src'.format(subdir) if subdir else 'src'
    modules = ['core', 'graphics', 'ios', 'math', 'osx', 'remote-control', 'utils', 'ns']
    rosewood_srcs = _filter_modules(bld.env.RW_MODULES, sources_in_modules(bld, rwdir, modules))
