...
: _projection_matrix(camera->projection_matrix()) {
    switch (camera->mode()) {
        case ProjectionMode::kPerspectiveMode:
            make_perspective_frustum_planes(camera);
            break;
        case ProjectionMode::kOrthographicMode:
            make_orthographic_frustum_planes(camera);
            break;
    }
}
...
void ViewFrustum::make_perspective_frustum_planes(const Camera *camera) {
...
void ViewFrustum::make_orthographic_frustum_planes(const Camera *camera) {
    Vector3 forward(0, 0, -1);
    Vector3 up(0, 1, 0);
    Vector3 right(1, 0, 0);

    Vector3 far_center = forward * camera->z_far();
    Vector3 far_tl = far_center + (up * camera->height()/2) - (right * camera->width()/2);
    Vector3 far_tr = far_center + (up * camera->height()/2) + (right * camera->width()/2);
    Vector3 far_bl = far_center - (up * camera->height()/2) - (right * camera->width()/2);
    Vector3 far_br = far_center - (up * camera->height()/2) + (right * camera->width()/2);

    Vector3 near_center = forward * camera->z_near();
    Vector3 near_tl = near_center + (up * camera->height()/2) - (right * camera->width()/2);
    Vector3 near_tr = near_center + (up * camera->height()/2) + (right * camera->width()/2);
    Vector3 near_bl = near_center - (up * camera->height()/2) - (right * camera->width()/2);
    Vector3 near_br = near_center - (up * camera->height()/2) + (right * camera->width()/2);
