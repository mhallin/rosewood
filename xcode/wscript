...
def build(bld):
    ...
    modules = ['core', 'graphics', 'ios', 'math', 'osx', 'remote-control', 'utils']
    example_modules = ['.', 'ios', 'osx', 'x11']

    enabled_modules = ['bsd', 'libpng']

    rosewood_srcs = _filter_modules(enabled_modules, sources_in_modules('src', modules))
    example_srcs = _filter_modules(enabled_modules, sources_in_modules('example', example_modules))
    test_srcs = _filter_modules(enabled_modules, sources_in_modules('tests', ['.']))
