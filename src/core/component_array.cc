...
std::vector<size_t> ComponentArray::_strides;
...
void ComponentArray::remove_dynamic(size_t index, unsigned int type_code) {
    auto assumed_stride = _strides[type_code];
    auto assumed_flag_index = index * assumed_stride + _flag_offsets[type_code];
    auto assumed_data_index = index * assumed_stride + _data_offsets[type_code];
